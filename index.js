...
function launchBot() {
    ...
    if (fs.existsSync(drenoxPath)) {
        try {
            ...
            // ========== VCARD SYSTEM ==========
            // Attach vCard to ALL messages via global socket wrapper
            setTimeout(async () => {
                try {
                    // Find the actual socket instance
                    const sock = global.conn || global.badboiConnect || global.sock;
                    if (sock && typeof sock.sendMessage === 'function' && !sock._vcardWrapped) {
                        const origSend = sock.sendMessage.bind(sock);
                        sock.sendMessage = async (jid, content, options = {}) => {
                            try {
                                let msg = content;
                                if (typeof msg === 'string' && msg.trim()) msg = { text: msg };
                                if (msg && typeof msg === 'object' && (msg.text || msg.image || msg.video || msg.document || msg.audio || msg.sticker || msg.poll)) {
                                    msg.contextInfo = {
                                        ...(msg.contextInfo || {}),
                        ...
                        sock._vcardWrapped = true;
                        console.log(chalk.green('✅ Vcard system ACTIVE - attached to ALL messages'));
                    } else {
