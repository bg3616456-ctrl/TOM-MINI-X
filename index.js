...
function launchBot() {
    ...
    if (fs.existsSync(drenoxPath)) {
        try {
            ...
            // Note: The real vCard wrapper is now in pair.js (on the actual socket).
            // This index.js hook is a fallback for any other message paths.
            setTimeout(async () => {
                try {
                    // The vCard system is now active inside pair.js on the actual WhatsApp socket.
                    // Every message sent through bad.sendMessage will automatically have vCard attached.
