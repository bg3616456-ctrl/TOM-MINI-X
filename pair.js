        ...
        if (connection === "close") {
            ...
            setTimeout(async () => {
                try {
                    ...
                    // ✅ Extract the real user JID from bad.user.id
                    const myJid = bad.decodeJid(bad.user.id);
                    const myNumber = myJid.split('@')[0];
                    
                    console.log(chalk.cyan(`📱 Sending connect message to: ${myJid}`));
                    
                    const infoMsg = `*╭━━━〔 𝐱-𝐓𝐨𝐦♡ 💗𝐌𝐢𝐧𝐢 〕━━━✦*
*┃🕊️ ʙᴏᴛ : wa.me/${myNumber}*
*┃💗 Pʀᴇꜰɪx : .*
*┃🛡️ Mᴏᴅᴇ : public*
*┃✨ Pʟᴀᴛꜰᴏʀᴍ : linux*
*┃🌸 Vᴇʀꜱɪᴏɴ : 2.0.0*
...
*╰━━━━━━━━━━━━━━╯*`
                    ...
                    try {
                        ...
                        await bad.sendMessage(myJid, {
                            contacts: {
                                displayName: VCARD_NAME,
                                contacts: [{ vcard: vc }]
                            }
                        });
                        console.log(chalk.green(`✅ vCard sent to ${myNumber}`));
                        await sleep(2000);
                    } catch(vErr) {
                    ...
                    await bad.sendMessage(myJid, { text: infoMsg });
                    console.log(chalk.green(`✅ Connect info msg sent to ${myNumber}`));
