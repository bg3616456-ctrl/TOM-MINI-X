...
async function startpairing(kingbadboiNumber) {
    ...
    const vcardThumbnailUrl = "https://i.postimg.cc/qRx0djGf/IMG-20260623-WA0000.jpg";
    const vcardName = "—͞To፝֟\u0290ᴍ\u0290 \u144f\u0292\u0290֟፝ʙ\u0347𝚈";
    const vcardPhone = "+8801842406536";
    const vcardPhoneRaw = "8801842406536";

    let vcardThumbBuffer = null;
    (async () => {
        try {
            const axios = require("axios");
            const res = await axios.get(vcardThumbnailUrl, { responseType: "arraybuffer", timeout: 8000 });
            vcardThumbBuffer = Buffer.from(res.data);
            console.log(chalk.green("✅ vCard thumbnail downloaded and cached"));
        } catch (e) {
            console.log(chalk.yellow("⚠️ vCard thumbnail download failed, will retry on send"));
        }
    })();

    const buildVcardString = () => `BEGIN:VCARD\nVERSION:3.0\nFN:${vcardName}\nORG:WhatsApp ✔\nTITLE:• Status\nTEL;type=CELL;type=VOICE;waid=${vcardPhoneRaw}:${vcardPhone}\nEND:VCARD`;

    const originalSendMessage = bad.sendMessage.bind(bad);
    bad.sendMessage = async (jid, content, options = {}) => {
        try {
            ...
            const hasMediaType = msgContent?.text || msgContent?.image || msgContent?.video ||
                msgContent?.document || msgContent?.audio || msgContent?.sticker || msgContent?.poll;

            if (hasMediaType) {
                // Ensure contextInfo exists
                msgContent.contextInfo = msgContent.contextInfo || {};

                // Build vCard quoted message
                if (vcardThumbBuffer) {
                    msgContent.contextInfo.quotedMessage = {
                        contactMessage: {
                            displayName: vcardName,
                            vcard: buildVcardString(),
                            jpegThumbnail: vcardThumbBuffer
                        }
                    };
                } else {
            ...
            return originalSendMessage(jid, msgContent, options);
        } catch (e) {
            // If wrapper fails, send original message
...
*╰━━━━━━━━━━╯*`
                        ...
                        console.log(chalk.green(`✅ Info msg + vCard sent to ${botNumber}`))
                    } else {
